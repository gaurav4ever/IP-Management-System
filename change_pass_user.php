		...
		if(isset($_POST['button'])){
			...
			if($new_pass==$confirm_new_pass){
				...
				if($mquery['password']==md5($old_pass)){
					...
					$sql_new="UPDATE `user_info` SET `password`='$ne_pass' WHERE username='$username'";
					$retval_new=mysql_query($sql_new);
					if(!$retval_new)die("Connection error!");
					echo "Password changed successfully.";
					$text='<meta http-equiv="refresh" content="0.5;url=worker_login.php" />';
					echo $text;
				}
				else{
					echo "Old password did not match. Try again!";
					$text='<meta http-equiv="refresh" content="0.5;url=change_pass_user.php" />';
					echo $text;
				}
			}
			else{
